...
- **发给 Agent 的 prompt**：含上述原则、文档标题、知识库、作者、原文地址、本地文件绝对路径（可读以生成概要），并明确要求 Agent 回调结构化 JSON；自定义模板见 `YUQUE2GIT_OPENCLAW_MESSAGE_TEMPLATE`，占位符见 [SKILL.md](SKILL.md)（含 `{push_policy}`、`{reply_contract}`）。
